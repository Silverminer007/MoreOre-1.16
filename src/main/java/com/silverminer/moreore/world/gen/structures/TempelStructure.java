...
public class TempelStructure extends AbstractStructure {
	...
	protected int getFeatureDistance(ChunkGenerator generator) {
		return StructureGenConfig.TEMPEL_DISTANCE.get();
	}

	@Override
	protected int getFeatureSeparation(ChunkGenerator generator) {
		return StructureGenConfig.TEMPEL_SEPARATION.get();
	}

	@Override
	protected double getSpawnChance() {
		return StructureGenConfig.TEMPEL_SPAWN_CHANCE.get();
	}
	...
	public static class Start<C extends IFeatureConfig> extends MarginedStructureStart<C> {
		...
		public void func_230364_a_(ChunkGenerator generator, TemplateManager templateManager, int chunkX, int chunkZ,
			...
			TempelPieces.generate(generator, templateManager, blockpos, this.components, this.rand);

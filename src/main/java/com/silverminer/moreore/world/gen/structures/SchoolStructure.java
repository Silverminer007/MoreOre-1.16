...
public class SchoolStructure extends AbstractStructure {
	public static final String SHORT_NAME = "school";

	public SchoolStructure(Codec<NoFeatureConfig> configFactoryIn) {
		super(configFactoryIn, SHORT_NAME);
	}

	@Override
	protected int getFeatureDistance(ChunkGenerator generator) {
		return StructureGenConfig.SCHOOL_DISTANCE.get();
	}

	@Override
	protected int getFeatureSeparation(ChunkGenerator generator) {
		return StructureGenConfig.SCHOOL_SEPARATION.get();
	}

	@Override
	protected double getSpawnChance() {
		return StructureGenConfig.SCHOOL_SPAWN_CHANCE.get();
	}
	...
	public static class Start<C extends IFeatureConfig> extends MarginedStructureStart<C> {
		...
		public void func_230364_a_(ChunkGenerator generator, TemplateManager templateManager, int chunkX,
				int chunkZ, Biome biome, C iconfig) {
			...
			SchoolPieces.generate(generator, templateManager, blockpos, this.components, this.rand);

...
public class SilverOverworldChunkGenerator extends NoiseChunkGenerator<SilverGenSettings> {
	...
	private final VillageSiege village_victory = new VillageSiege();

	public SilverOverworldChunkGenerator(IWorld worldIn, BiomeProvider provider, SilverGenSettings settingsIn) {
	...
	protected double[] getBiomeNoiseColumn(int noiseX, int noiseZ) {
		...
		for (int k = -i; k <= i; ++k) {
			for (int l = -i; l <= i; ++l) {
				...
				float f6 = field_222576_h[k + i + (l + i) * 5] / (f4 + 2.0F);
	...
	public void spawnMobs(ServerWorld worldIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs) {
		...
		this.village_victory.tick(worldIn, spawnHostileMobs, spawnPeacefulMobs);
	}

...
public class NutBushBlock extends SweetBerryBushBlock {
	public static final IntegerProperty AGE = BlockStateProperties.AGE_0_3;

	public NutBushBlock(Block.Properties properties) {
		super(properties);
	}

	@Override
	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		return new ItemStack(FootItems.NUTS.get());
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
			...
			Block.spawnAsEntity(worldIn, pos, new ItemStack(FootItems.NUTS.get(), j + (flag ? 1 : 0)));

package com.silverminer.moreore.common.portal.registration;
...
public class Portal implements INBTSerializable<CompoundNBT>
{
	...
	public boolean isDamaged(World world)
	{
		...
		{
			if (!(world.getBlockState(pos).getBlock() instanceof SilverPortalFrameBlock)) return true;
		}
		
		for (BlockPos pos : getPortalPositions())
		{
			if (!(world.getBlockState(pos).getBlock() instanceof SilverPortalBlock)) return true;
		}
